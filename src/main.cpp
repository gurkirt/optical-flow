...
float computeFlowForImages(string& inputdir, string& outdir, string& format, int method, int verbosity, int device_id, int bound)
{
	...
	Ptr<cuda::OpticalFlowDual_TVL1> tvl1 = cuda::OpticalFlowDual_TVL1::create();  // instantiate TVL1 optical flow object
	...
	for(frame_num = 0; frame_num<nb_frames; frame_num++) {
		...
		if (!chekcIfFileExists(outfilename))
		{
			...
			{ 
				...
				dstSize.height = int(prev_image.rows/dscale);
				dstSize.width = int(prev_image.cols/dscale);
				...
				cv::resize(next_image,next_image,dstSize);
				...
				next_image = cv::imread(inputdir+filelist[frame_num+1], IMREAD_GRAYSCALE);
				dstSize.height = int(next_image.rows/dscale);
                                dstSize.width = int(next_image.cols/dscale);
				cv::resize(next_image,next_image,dstSize);
			}
...
int computeFlowVideos(string& inputdir, string& outdir, string& format, int method, int verbosity, int device_id, int bound, int start, int end)
{
	...
	{
		string inputBasedir = inputdir+dirlist[i]+"/";
		string outBasedir = outdir+dirlist[i]+"/";

		float avg_time =  computeFlowForImages(inputBasedir, outBasedir, format, method, verbosity, device_id, bound);
		if(verbosity>0) cout<<"Done computting for directory number "<<i<<" with avergae time of "<<avg_time<<" seconds per image"<<endl<<endl;

	}
}

int computeFlowVideosOnActions(string& inputdir, string& outdir, string& format, int method, int verbosity, int device_id, int bound, int start, int end)
{
	std::vector <std::string> actiondirlist = listdir(inputdir);
	for(int i=actiondirlist.size()-1;i>=0;i--)
	{
		...
		//for(int i=start; i<end; i++)
		{
			string inputBasedir = actiondir+dirlist[i]+"/";
			string outBasedir = outactiondir+dirlist[i]+"/";
...
int main(int argc, char **argv)
{
	...
	const char* keys =
	{
			...
			"{ act   action     | 1 | if directory structure is based on action}"
			"{ id  inputDir   | /mnt/earth-alpha/yolo-data/images/ | path of directory which contain images of folders with images where each folder belog to one video }"
			"{ od  outputDir  | /mnt/jupiter-beta/actnet/motion-images/ | Name of the directory where flow images will be stroed }"
			"{ f   format     | .jpg | format of input images}"
			"{ b   bound      | 8 | specify the maximum of optical flow}"
			"{ d   device_id  | 0  | set gpu id}"
			"{ v   verbosity  | 1  | set gpu id}"
			"{ sv  start  | 0  | set gpu id}"
			"{ ev  end  | 19994  | set gpu id}"
	};
	...
	if (isact>0) {computeFlowVideosOnActions(inputBasedir, outBaseDir, format, method, verbosity, device_id, bound, start, end);}
